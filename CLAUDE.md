...
- **There are real users on the other side of this.** Error messages, README copy, and issue replies
  matter as much as the code. See `plinth/planning/revival-plan.md` §6 for the tone.
...
Start with `plinth/planning/revival-plan.md`. It is the binding plan until the NLSpec supersedes it.
...
- **Callers supply their own Morphemeris API key.** No shipped shared key, no free proxy — both
  recreate the failure being fixed. See revival-plan §4.1.
- **Local computation stays local.** Morphemeris provides ephemeris and houses; aspects, synastry,
  ...
  default path. See revival-plan §3.
...
- `~/dev/morphemeris/` — the API this library consumes. Its NLSpec is the authority on endpoint
  shapes, auth, and rate limits. `mcp/` there is the toolchain precedent for this repo.
- `~/dev/swephrs/` — the Rust ephemeris engine underneath Morphemeris.
- `~/dev/astrologica/` — sibling astrology work.
