...
class Chart {
    constructor(name, p1, cdata, p2, type = ChartType.Basic) {
         ...
         */
        this.x = (radius, lon) => {
            return radius * Math.cos(chart_factory_1.ChartFactory.toRadians(180 + this._ascendant - lon));
        };
        /**
         ...
         */
        this.y = (radius, lon) => {
            return radius * Math.sin(chart_factory_1.ChartFactory.toRadians(180 + this._ascendant - lon));
        };
        this.sign = (lon) => this._signs[Math.floor(lon / 30)];
        this.degMinSec = (lon) => {
            let deg, min, sec, sign;
            sign = this.sign(lon);
            lon = lon % 30;
            deg = Math.floor(lon);
    ...
    getD3Data(innerRadius, outerRadius) {
        let radius = this._planets2 ? innerRadius : outerRadius, inOrOut = this._planets2 ? "innerPlanets" : "outerPlanets", data = {
            "name": this.name,
            "innerPlanets": [],
            "outerPlanets": [],
            "aspects": [],
